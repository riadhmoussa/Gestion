...
class MyTimePicker : DialogFragment(){
    ...
    private var onTimeSetOption:
                (hour: String, minute: String, second: String) -> Unit = {_, _, _ -> }
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let {
            ...
            builder.setPositiveButton(timeSetText) { _, _ ->
                ...
                var min = ""
                var sec=""
                if (hour < 10) heur = "0" + hour
                else heur = "" + hour

                if (minPicker.value < 10) min = "0" + minPicker.value
                else min = "" + minPicker.value
                if (secPicker.value < 10) sec = "0" + secPicker.value
                else sec = "" + secPicker.value
                onTimeSetOption(heur, min, sec)
            }
     ...
     */
    fun setOnTimeSetOption (text: String, onTimeSet: (hour: String, minute: String, second: String) -> Unit) {
